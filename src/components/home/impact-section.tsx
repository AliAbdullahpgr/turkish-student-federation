      ...
      <SectionFrame>
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-12 md:grid-cols-3 lg:grid-cols-6">
          {impact.items.map((item, index) => (
            <div
              key={item.label}
              className={`reveal-up flex flex-col items-center space-y-4 text-center reveal-delay-${(index % 4) + 1}`}
            >
              {/* Circular Icon */}
              ...
              <p className="font-headline text-3xl font-extrabold tracking-[-0.05em] text-secondary-fixed-dim sm:text-4xl">
                {item.value}
              </p>
